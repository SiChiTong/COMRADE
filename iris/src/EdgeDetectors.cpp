...
void Comrade::IrisXT::EdgeDetectorManager::Canny(Buffer<RGB>* ip_buffer,
                                                 ...
                                                 double sigma)
{
    Buffer<RGB> temp_op(ip_buffer->maxx,ip_buffer->maxy);
    ...
    op.assign_buffer_ptrs(ip_buffer,&temp_op);
    op.set_factor(1);
	op.convolve();
    ...
    for (int yy=1; yy<=ip_buffer->maxy-2; ++yy)
    {
        ...
        {
            op.assign_kernel(&h1);
            double sumX=op.greyscale_unit_convolve(xx,yy,&temp_op);

            op.assign_kernel(&h2);
            double sumY=op.greyscale_unit_convolve(xx,yy,&temp_op);
            ...
            if(edge_direction==0)
            {
                leftPixel=temp_op.at(xx,yy-1).green;
                rightPixel=temp_op.at(xx,yy+1).green;
            }

            else if(edge_direction==45)
            {
                leftPixel=temp_op.at(xx-1,yy-1).green;
                rightPixel=temp_op.at(xx+1,yy+1).green;
            }

            else if(edge_direction==90)
            {
                leftPixel=temp_op.at(xx+1,yy).green;
                rightPixel=temp_op.at(xx-1,yy).green;
            }
            ...
            else
            {
                ...
                {
                    double l1=temp_op.at(xx-1,yy-1).green;
                    double l2=temp_op.at(xx,yy-1).green;
                    double l3=temp_op.at(xx+1,yy-1).green;
                    double l4=temp_op.at(xx+1,yy).green;
                    double l5=temp_op.at(xx+1,yy+1).green;
                    double l6=temp_op.at(xx,yy+1).green;
                    double l7=temp_op.at(xx-1,yy+1).green;
                    double l8=temp_op.at(xx-1,yy).green;
            ...
            op_buffer->at(xx,yy).red=
            op_buffer->at(xx,yy).green=
            op_buffer->at(xx,yy).blue=static_cast<unsigned char>(sum);
        }
    }
}
