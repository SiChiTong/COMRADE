 ...
 Comrade::IrisFoundation::Buffer<RGB>* buf)
{
    ...
    if (greyscale_is_consistent(current->data,buf)==false)
    {
...
bool Comrade::IrisFoundation::QuadtreeSegmenter::greyscale_is_consistent
(Comrade::IrisFoundation::Rectangle rect,
 Comrade::IrisFoundation::Buffer<RGB>* buf)
{
	unsigned char min=buf->at(rect.x1,rect.y1).green,
				  max=buf->at(rect.x1,rect.y1).green;
	
 	for (int yy=rect.y1; yy<=rect.y2; ++yy)
    {
        for (int xx=rect.x1; xx<=rect.x2; ++xx)
        {
			if (buf->at(xx,yy).green<min)
			{
				min=buf->at(xx,yy).green;
			}
			
			if (buf->at(xx,yy).green>max)
			{
				max=buf->at(xx,yy).green;
			}
		}
	}
...
bool Comrade::IrisFoundation::belongs
(Comrade::IrisFoundation::Rectangle rect,Comrade::Iris3D::Coordinate coord)
{
