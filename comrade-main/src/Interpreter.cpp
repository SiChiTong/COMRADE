...
Scorpio::Interpreter::Interpreter()
{
    //cout<<"Interpreter object created...\n";
}

void Scorpio::Interpreter::interpret(const char* cmd)
{
    if (strlen(cmd)!=0)
    {
...
void Scorpio::Interpreter::add
(std::string cmd_string,std::string form,function_handle fn)
{
    binder.add(cmd_string,form,fn);
}

void Scorpio::Interpreter::erase(std::string cmd_string)
{
    binder.erase(cmd_string);
}

void Scorpio::Interpreter::match_with_command(char s[20])
{
    ...
    {
        stream.write(tok);
        stream.write(":Unrecognised command.\n");
    }
...
void Scorpio::Interpreter::parse_for_arguments
(Scorpio::CommandObject& cmd_obj)
{
...
void Scorpio::Interpreter::store_current()
{
    //cout<<"Problem start...\n";
...
void Scorpio::Interpreter::restore_current()
{
    if (cmd_stack.size()>0)
    {
